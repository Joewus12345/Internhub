...
export async function StudentsignUpAction(data: StudentSignUpInput) {
  ...
  try {
    ...
    const transport = await emailTransporter.sendMail({
      from: '"InternHub" <joewus12345@gmail.com>',
      to: email,
    ...
    console.log("Email sent successfully:", transport.messageId);
    return { message: "valid" };
  } catch (error) {
...
export async function resetOtpAction(email: string, otp: string, role: string) {
  try {
    // 1) Find user by role
    let foundUser = null;
    if (role === "student") {
      ...
      foundUser = await prisma.companyRepresentative.findUnique({
        where: { email, otp },
      });
    }

    // If user not found => invalid OTP
    if (!foundUser) {
      await emailTransporter.sendMail({
        from: '"InternHub" <noreply@internhub.com>',
        to: email,
        subject: "InternHub Password Reset Attempt Failed",
        html: `
        <!DOCTYPE html>
        <html lang="en">
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>Password Reset Attempt Failed</title>
            <style>
                body {
                    font-family: Arial, sans-serif;
                    background-color: #f4f4f4;
        ...
        <body>
            <div class="container">
                <div class="content">
                    ...
                    <p>Hello,</p>
                    <p>We received a request to reset your password on InternHub, 
                       but the OTP provided was invalid.</p>
                    <p>If you did not initiate this request, please ignore this email or contact support.</p>
                    <p>Thank you,</p>
                    <p>The InternHub Team</p>
                </div>
                <div class="footer">
                    <p>Need help? Please reach out to our support team.</p>
                </div>
            </div>
        </body>
        </html>
      ...
      return { message: null }; // or { message: "OTP invalid" }
    }

    // If user found => valid OTP
    // 2) Send success email (but do NOT update password here)
    await emailTransporter.sendMail({
      from: '"InternHub" <noreply@internhub.com>',
      to: email,
      subject: "InternHub Password Reset Successful",
      html: `
      <!DOCTYPE html>
      <html lang="en">
      <head>
      ...
      <body>
          <div class="container">
              <div class="content">
                  ...
                  <p>Hello,</p>
                  <p>Your password on InternHub has been reset successfully.</p>
    ...
    return { message: "OTP valid, email sent for successful reset." };
  } catch (error) {
    console.log("Error in resetOtpAction:", error);
    return { message: null };
  }
}
...
export async function generatePdfAction() {
  try {
    ...
    const pdfBytesAsUint8Array = new Uint8Array(existingPdfBytes);

    // Load the PDF with pdf-lib
    const pdfDoc = await PDFDocument.load(pdfBytesAsUint8Array);
...
export async function generateAcceptPdfAction(id: string) {
  try {
    ...
    if (!application)
      return { success: false, message: "application does not exits" };
    // Load the existing PDF template
    ...
    firstPage.drawText(
      formatDateToCustomString(
        application.internship?.start_date?.toISOString() || ""
      ),
      {
        x: 218,
        y: 527,
    ...
    firstPage.drawText(application.internship.title + ".", {
      x: 169,
      y: 549,
      size: 11,
      font: timesFont,
...
export async function SoftDeletAction(id: string) {
  try {
    ...
    return {
      success: false,
      message: "An error occurred while deleting the posted opening",
    };
  }
}
