      ...
      <div className="flex mt-5 justify-center min-h-screen">
        <ScrollArea className="h-full max-w-96 mx-7 sm:max-md:min-w-96">
          <div className="flex flex-col gap-4 p-4 pt-0">
            {!isloading && Array.isArray(data) ? (data.map((item) => (<PostCard key={item.id} item={item} />))) : null}
            {isloading &&
        ...
        <PostDetail
          post={
            (!isloading && Array.isArray(data) ? (data.find((item) => item.id === post.selected)) : null) || null
          }
        />
      </div>
    </>
