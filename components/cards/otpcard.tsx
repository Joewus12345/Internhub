...
import { useResetInfo, useResetPassword } from "@/utils/state"
import toast, { Toaster } from "react-hot-toast"

export function InputOTPControlled() {
  const [resetInfo, setResetInfo] = useResetInfo()
  const [resetKey, setResetkey] = useResetPassword()

  async function handleSubmit() {
    const response = await resetOtpAction(
      resetInfo.email,
      resetInfo.otp,
      resetInfo.role
    )
    ...
    if (response?.message) {
      setResetkey(true)
      toast.success("OTP was valid! Check your email for confirmation.")
    } else {
      setResetkey(false)
