...
const PostCard = ({ item }: PostProps) => {
  ...
  const capitalizedLocation = item?.location
      ...
      <div className="flex w-full flex-col gap-1">
        <div className="flex items-center">
          <div className="flex items-center gap-2">
            <div
              className={`font-semibold text-xl ${hoverState && 'underline'}`}
              onClick={(e) => {
                e.stopPropagation()
                router.push(`/student/dashboard/${item.id}`)
              }}>{capitalizeFirstLetter(item.title)}</div>
          </div>
        </div>
