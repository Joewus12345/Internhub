...
import { otpVerifyAction, resendOtpAction, resetOtpAction } from "@/app/action";
...
const ResetModal = ({
  ...
  async function handleSubmit() {
    ...
    if (res?.message === "valid") {
      const resetResult = await resetOtpAction(email, otp, role);
      if (resetResult?.message) {
        toast.success("OTP verified & reset email sent. You can now proceed")
        setTimeout(() => {
          setState(true);
          handleCloseModal();
        }, 2000);
      } else {
        toast.error("OTP verified but reset email failed. Please try again.");
      }
