...
const ApplicationModal = ({ isModalOpen, handleCloseModal, postid }: Props) => {
  ...
  return (
    <div className="flex items-center justify-center relative z-50">
      ...
      >
        {/* Modal content */}
        <div className="bg-white rounded-sm md:w-1/2 h-96 relative">
          <div
            onClick={handleCloseModal}
            className="cursor-pointer absolute top-3 left-16 xs:left-6"
          >
            <CircleX />
          </div>
          <form
            className="flex items-start justify-center gap-2 px-20 md:px-24 h-full flex-col"
            onSubmit={handleSubmit}
          >
            <h2 className="w-[300px]">Your message to employer</h2>
            <Textarea
              onChange={(e) => {
